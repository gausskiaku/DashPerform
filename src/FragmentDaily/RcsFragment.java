...
public class RcsFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater,
		...
		txtDATE = (TextView) view.findViewById(R.id.txtDateUpdateRCS);
...
//		--------------------------------------------------------------------------------------------
		try {
			...
			if(ninfo != null && ninfo.isConnected()){
				...
				Cursor listNet = network_KPI.getRCSKPI(getActivity());
			    int ii = 0;
...
//			    Toast.makeText(getBaseContext(), "ok", Toast.LENGTH_LONG).show();
				...
				while(listNet.moveToNext()){
			        entriesCDR.add(new Entry(100 - (float) listNet.getDouble(3),ii));
			        entriesCSSR.add(new Entry((float) listNet.getDouble(4),ii));
