...
public class ThreeGFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater,
		...
		txtDATE = (TextView) view.findViewById(R.id.txtDateUpdate3G);
...
//		--------------------------------------------------------------------------------------------
			...
			try {
				...
				if(ninfo != null && ninfo.isConnected()){
					...
					Cursor listNet = network_KPI.get3gKPI(getActivity());
				    int ii = 0;
					...
					while(listNet.moveToNext()){
				        entriesCDR.add(new Entry(100 - (float) listNet.getDouble(3),ii)); // CDR
				        entriesCSSR.add(new Entry((float) listNet.getDouble(4),ii)); // CSSR
				        entriesAVAIL.add(new Entry((float) listNet.getDouble(5),ii));
				        date = listNet.getString(0);
				        try {
				        	String val = String.valueOf(listNet.getDouble(5)).toString();
