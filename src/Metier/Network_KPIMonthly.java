...
public class Network_KPIMonthly {
	...
	public List<Network> getAll(Context context){
		...
		try {
			...
			ResultSet query = state.executeQuery("SELECT Historique.Datetime, SUM(Erlang) AS traffic, SUM(Revenu) AS revenu, SUM(CDR) AS cdr, SUM(CSSR) AS cssr, SUM(Avail) AS avail FROM Historique WHERE Historique.Datetime BETWEEN DATEADD(DAY, -31, GETDATE()) AND GETDATE() GROUP BY Historique.Datetime");
